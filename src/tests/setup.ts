...
// Mock the PropertyValuationData class globally
vi.mock('@/data/property-valuation-data', () => {
  // Create prototype-like mock object with all methods that tracks when it was instantiated
  const mockInstance = {
  ...
  const MockPropertyValuationData = vi.fn().mockImplementation(() => mockInstance);
  
  // Create a type for the mock constructor that includes the mockInstance property
  type MockPropertyValuationDataType = typeof MockPropertyValuationData & {
    mockInstance: typeof mockInstance;
  };
  
  // Add the instance for test access and cast to the proper type
  (MockPropertyValuationData as MockPropertyValuationDataType).mockInstance = mockInstance;
  
  return {
    PropertyValuationData: MockPropertyValuationData
  };
}); 